...
private object UnlimitedIoScheduler : CoroutineDispatcher() {
    ...
    override fun limitedParallelism(parallelism: Int, name: String?): CoroutineDispatcher {
        parallelism.checkParallelism()
        // TODO fix this one, inconsistent toString()
        if (parallelism >= MAX_POOL_SIZE) return this
        // For the debugging purposes we do not pretend that UnlimitedIoScheduler is Dispatchers.IO
        return super.limitedParallelism(parallelism, name ?: "Dispatchers.IO.limitedParallelism($parallelism)")
    }

    override fun toString(): String {
        return "UnlimitedIoScheduler"
    }
}
